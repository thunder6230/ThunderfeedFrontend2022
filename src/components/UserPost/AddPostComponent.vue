...
const handleDeleteFile = (indexToDelete:number) => {
  ...
  formDataFiles.forEach(file => filteredFormData.append("file", file, file.name))
...
<template>
  <form :class="TailwindClasses.ADD_POST_ELEMENT_STYLE" @submit.prevent="handleSubmit()" enctype="multipart/form-data">
    <div class="relative">
      <input type="text" :class="TailwindClasses.INPUT_STYLE" class="w-full" style="margin: 0;margin-right: 0.5em;"
             v-model="postBody">
      <div class="absolute flex right-3.5 -inset-y-1/2">
        <label for="file" class=" cursor-pointer flex items-center justify-center">
          <font-awesome-icon icon="file-image" :class="TailwindClasses.ADD_POST_BUTTONS_STYLE" class="p-2" />
          <input type="file" multiple name="" id="file" class="hidden" @change="onFileChange($event.target.files)">

        </label>
        <div class=" flex items-center justify-center">
          <button type="submit" class="w-full flex items-center justify-center">
            <font-awesome-icon icon="message" :class="TailwindClasses.ADD_POST_BUTTONS_STYLE" />
          </button>
    ...
    <TransitionGroup name="list" tag="ul">
      <AddPostFileListComponent v-for="(file,index) in files" :key="file" :file="file" :index="index" @deleteFile="handleDeleteFile($event)"/>
    </TransitionGroup>
